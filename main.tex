...
\setbeamertemplate{title page}{
	...
	\if\fullversion0
	\vskip2em
	{Full version of the presentation:\par}
	{\textcolor{LightBlueZB}{\href{https://github.com/ZeroBone/LuksAlgoP/releases/latest}{github.com/ZeroBone/LuksAlgoP/releases/latest}}\par}
	\fi
}
...
\begin{frame}{Graph Isomorphism Problem}
	...
	An \textbf{isomorphism} between undirected graphs \( G_0 = (V_0, E_0) \) and \( G_1 = (V_1, E_1) \) is a bijection $ \sigma : V_0 \to V_1 $ such that \[
	\{u,v\} \in E_0 \iff \{\sigma(u), \sigma(v)\} \in E_1
	\]\vspace{-15pt}\begin{itemize}
		\item Let $ \Iso(G_0, G_1) $ denote the set of all isomorphisms between $ G_0 $ and $ G_1 $
		\item $ G_0 $ and $ G_1 $ are \textbf{isomorphic} (notation: $ G_0 \cong G_1 $) if $ \Iso(G_0, G_1) \neq \varnothing $
	\end{itemize}
	
	\textbf{Example}: \begin{center}
...
\begin{frame}{$ \GI $ has a very special complexity status (for all we know)}
	\begin{center}
...
\begin{frame}{Automorphism groups of graphs}
	...
	\begin{itemize}
		...
		\item To see this, consider first $ \Aut(G) := \Iso(G, G) $ for a fixed graph $ G $
	\end{itemize}
...
\begin{frame}{Set of isomorphic graphs is a set of cosets}
	\begin{itemize}
		...
		\]
		\item $ \rightsquigarrow $ In this sense, solutions to $ \GI $ are sets of cosets!
	\end{itemize}
\end{frame}

\fi

\begin{frame}{Set of graph isomorphisms is a coset}
	\begin{itemize}
		\item Fix graphs $ G_0 $ and $ G_1 $ and let $ \sigma \in \Iso(G_0, G_1) $
		\item $ \rightsquigarrow $ For any $ \tau \in \Aut(G_0) $, $ \sigma\tau $ is also an isomorphism:\begin{center}
			...
			\end{tikzpicture}
		\end{center}
		\item There are no further solutions because for any $ \sigma_1, \sigma_2 \in \Iso(G_0, G_1) $\begin{center}
			\begin{tikzpicture}[thick]
				\node (g0) {$ G_0 $};
				\node[right=4 of g0] (g1) {$ G_1 $};
		...
		\item $ \rightsquigarrow \Iso(G_0, G_1) $ is a coset of the form $ \sigma \Aut(G_0) $
	\end{itemize}
\end{frame}
...
\begin{frame}{High-level overview}
	...
	\begin{center}
		\begin{tikzpicture}[thick]
			...
			\draw[->, OliveGreenZB] (si) --node[midway, right]{solve} (algo);
			
			\draw [LightBlueZB, decorate, decoration = {brace, raise=5pt, amplitude=5pt, mirror}] (gi.north west) -- (si.south west) node[pos=0.5,left=10pt,black]{\blocktext{Encode graphs into strings, introduce group}};
			\draw [OliveGreenZB, decorate, decoration = {brace, raise=5pt, amplitude=5pt}] (si.north east) -- (algo.south east) node[pos=0.5,right=10pt,black]{\blocktext{Use group-theoretic machinery}};
		\end{tikzpicture}
	\end{center}
\end{frame}
...
\begin{frame}{Strings and permutations}
	...
	\textbf{Intuition}: $ d \in D $ is a position, $ u(d) $ is the character at that position.
...
\begin{frame}{Groups and their composition factors}
	...
	\vspace{-5pt}\begin{theorem}[Jordan-H{\"o}lder]
		Every finite group has a composition series. Moreover, the composition factors are unique up to isomorphism and ordering.
	\end{theorem}
...
\begin{frame}{$ \GammaD $-groups}
	...
	The class $ \GammaD $ has a lot of nice properties, in particular:
...
\begin{frame}{Effectivizing Luks's theorem}
	...
	\textbf{Crux of proof}: Combine the level-driven construction in the above proof with the reduction $ \GI \le_m^p \SI $ we showed earlier
	...
	\vspace{-3pt}\scalebox{0.84}{
		\begin{minipage}{\linewidth}
			\begin{algorithm}[H]
				...
				\KwOut{Coset $ \{\delta \in \Gamma\sigma \mid u^\delta(\omega) = v(\omega)\ \forall \omega \in \Omega\} $ or $ \varnothing $}
...
\begin{frame}{Analysis of case when $ \Gamma $ acts transitively on $ \Omega $}
	\begin{theorem}[Babai, Cameron, P\'{a}lfy]
		For every $ d \ge 1 $ there exists $ c $ such that for every set $ \Omega $, group $ \Gamma \in \GammaD $ and action $ \cdot $ of $ \Gamma $ on $ \Omega $ having minimal block $ \Delta $ it holds that \[
			\abs{\Gamma/\KernelOf(\psi_\Delta)} \le n^c
		\] where $ n := \abs{\{\gamma \cdot \Delta \mid \gamma \in \Gamma\}} $ is the number of blocks.
	\end{theorem}
	\begin{itemize}

...
\begin{frame}{Proof of Luks's theorem (continued)}
	...
	\textbf{Inductive step}:\begin{itemize}
		...
		\item By IH, $ \Aut(R_{i-1}, v) \in \GammaD $. Hence, $ \Aut(G[V(R_{i-1})], v) \in \GammaD $
		...
		\item By FHT, \[
		\Aut(R_i, v)/\KernelOf(\varphi_i) \cong \ImageOf(\varphi_i) = \Aut(G[V(R_{i-1})], v) \in \GammaD
		\]
...
\begin{frame}{Luks's algorithm for graph isomorphism}
	...
	\begin{lemma}
		Luks's algorithm for string isomorphism for $ \GammaD $-groups, where $ d $ is a constant, runs in polynomial time.
	\end{lemma}

...
\begin{frame}{Proof of Luks's theorem (continued)}
	...
	\textbf{Inductive step}:\begin{itemize}
		...
		\item $ \rightsquigarrow \Aut(G[V(R_{i-1})], v) \le \Aut(R_{i-1}, v) $
		\item By IH, $ \Aut(R_{i-1}, v) \in \Gamma_d $. Hence, $ \Aut(G[V(R_{i-1})], v) \in \Gamma_d $
		...
		\]
		\item By above lemma, it suffices to prove $ \KernelOf(\varphi_i) \in \Gamma_d $
	\end{itemize}
\end{frame}
...
\begin{frame}{Sketch of proof of Babai-Luks theorem (continued)}
	\begin{columns}
		\begin{column}{0.35\textwidth}
			\input{figures/babai_luks_reduction_proof}
		\end{column}
		\begin{column}{0.65\textwidth}
			\begin{itemize}
				\item TODO
...
\begin{frame}{Case when $ \Gamma $ acts intransitively on $ \Omega $}
	...
	Let $ \omega_1, \dots, \omega_s $ be a transversal of $ \{\Gamma \cdot \omega \mid \omega \in \Omega\} $ (note: $ s \ge 2 $)
	\begin{center}
		\input{figures/luks_intransitive_case}
	\end{center}
	\vspace{-5pt}\textbf{Note}: $ \Gamma \ge \Gamma_1 \ge \Gamma_2 \ge \dots \ge \Gamma_{s-1} $
\end{frame}
...
\newcommand{\NumLeaves}{\mathrm{\#Leaves}}
...
\begin{frame}{Time complexity analysis -- transitive case}
	...
	\] \textbf{Proof}:\begin{itemize}
		...
		\]
		%\item By IH, $ \NumLeaves(\underbrace{\strisoluks(u, v, \KernelOf(\psi_\Delta)t, \Omega)}_{\Theta_t\sigma_t}) \le \abs{\KernelOf(\psi_\Delta)}^{c+1} $
	\end{itemize}
\end{frame}
...
\begin{frame}{Time complexity analysis -- transitive case (continued)}
	...
	\] \textbf{Proof}: The tree of recursive calls starting at $ \Theta_t\sigma_t $ will be (in the worst case) of the form:\begin{center}
		\begin{tikzpicture}[thick, scale=1]
			...
			% first layer
			\node (n2) at ($ (root) + (-1.5, -1) $) {$ (\Gamma_1\sigma_1, \Delta_2 \cup \dots \cup \Delta_k) $};
			...
			\draw [decorate, decoration = {brace, raise=5pt, amplitude=5pt}] (n2grd2.north east) -- (nkgr.south east) node[pos=0.7,left=-18pt,black,rotate=-90]{$ k $ nodes};
		\end{tikzpicture}
	\end{center}
	\begin{itemize}
		\item The domain at every step is $ \le \abs{\Omega}/k $
		\item By IH, every end node has $ \le (\abs{\Omega}/k)^{c+1} $ leaves
...
\begin{frame}{Luks's algorithm for graph isomorphism}
	...
	\begin{lemma}
		Luks's algorithm for string isomorphism for $ \Gamma_d $-groups, where $ d $ is a constant, runs in polynomial time.
	\end{lemma}
	
	Combining this result with the Babai-Luks reduction from $ \GI $ to $ \SI $ for $ \Gamma_d $-groups yields:

...
\begin{frame}{Luks's algorithm for graph isomorphism}
	...
	Above claim, together with the observation that the degree of every non-leaf recursion call tree node or its child is $ \ge 3 $, yields:
	
	\begin{lemma}
		Luks's algorithm for string isomorphism for $ \Gamma_d $-groups, where $ d $ is a constant, runs in polynomial time.
	\end{lemma}
	
	By combining this result with the Babai-Luks reduction from $ \GI $ to $ \SI $ for $ \Gamma_d $-groups, we obtain:

...
\begin{frame}{Solutions to $ \GI $ are cosets}
	\begin{itemize}
		\item Let $ \cdot $ denote the vertex renaming operation of $ \Sym(V(G)) $ on the set of all graphs with vertices $ V(G) $
...
\begin{frame}{Blocks and block systems}
	...
	\begin{center}
		...
		\end{columns}
	\end{center}
	\textbf{Running example} -- block systems of non-trivial blocks:
...
\begin{frame}{Primitive actions and groups}
	\begin{itemize}
		\item A block $ \Delta $ (and $ \Delta $'s block system) is called \textit{minimal} if there is no non-trivial block $ \Delta' \supseteq \Delta $

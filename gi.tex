...
\begin{frame}{Flattening blocks of block-induced actions}
	Any block of $ \psi_\Delta $ induces a block of $ \cdot $ via union, for example:
	\begin{center}
		\begin{tikzpicture}[scale=.8, thick]
			\begin{scope}[shift={(-4,0)}]
				...
				\node[below=0 of l5] (blocksystemofdot1) {$ \underbrace{\{\textcolor{red!80}{\underbrace{\{1,5\}}_{\Delta}},\textcolor{OliveGreenZB!80}{\underbrace{\{2,6\}}_{r\cdot\Delta}},\textcolor{blue!80}{\underbrace{\{3,7\}}_{r^2 \cdot\Delta}}, \textcolor{DarkOrangeZB}{\underbrace{\{4,8\}}_{r^3 \cdot \Delta}}\}}_{\text{Block system of}\ \cdot} $};
			\end{scope}
			\begin{scope}[shift={(4,0)}]
				% octagon vertices
				\coordinate (v1) at ({90 - (0)*45}:2);
				...
				\filldraw[OliveGreenZB!80] (v6) circle (3pt);
				% third set
				\draw[very thick, red!80] (v3)--(v7);
				\filldraw[red!80] (v3) circle (3pt);
				\filldraw[red!80] (v7) circle (3pt);
				% fourth set
				\draw[very thick, OliveGreenZB] (v4)--(v8);
				\filldraw[OliveGreenZB] (v4) circle (3pt);
				\filldraw[OliveGreenZB] (v8) circle (3pt);
				
				% r symmetry
				\draw[->, bend left=20, gray] (l1) to node[midway, above] {$ r $} (l2);
				
				\node[below=0 of l5] (blocksystemofdot2) {$ \underbrace{\{\textcolor{red!80}{\underbrace{\{1,3,5,7\}}_{\Delta'}},\textcolor{OliveGreenZB!80}{\underbrace{\{2,4,6,8\}}_{r\cdot\Delta'}}\}}_{\text{Block system of}\ \cdot} $};
			\end{scope}
	...
	\scalebox{0.85}{
		\begin{minipage}{\linewidth}
			\begin{algorithm}[H]
				...
				\KwOut{$ \{\delta \in \Gamma\sigma \mid u(\omega) = v(\omega^\delta)\ \forall \omega \in \Omega\} $ represented as a coset}
				\SetKwFunction{luks}{striso}
				\SetKwProg{luksproc}{Function}{}{end}
				\luksproc{\luks{$ u $, $ v $, $ \Gamma\sigma $, $ \Omega $}}{
					\nl\If{$ \sigma \neq \id $}{
						\nl \Return $ \luks(u, v^{\sigma^{-1}}, \Gamma, \Omega) \sigma $\;
					}
					\nl\If{$ \abs{\Omega} = 1 $}{
						\nl Let $ \omega $ be such that $ \Omega = \{\omega\} $\;
						\nl \Return $ \Gamma $ if $ u(\omega) = v(\omega) $ and $ \varnothing $ otherwise\;
					}
					\nl\If{the action of $ \Gamma $ on $ \Omega $ is not transitive}{
						\nl Compute an orbit $ \Omega' \subsetneq \Omega $\;
						\nl $ \Gamma'\sigma' := \luks(u, v, \Gamma, \Omega') $\;
						\nl \Return $ \luks(u, v, \Gamma'\sigma', \Omega \setminus \Omega') $\;
					}
					...
					\nl Compute $ \KernelOf(\psi_\Delta) = \bigcap_{i=1}^k \Stab_{\psi_\Delta}(\Delta_i) $\;
					\nl Compute a transversal $ T $ of $ \KernelOf(\psi_\Delta) $ in $ \Gamma $\;
					\nl \Return $ \bigcup_{t \in T} \luks(u, v, \KernelOf(\psi_\Delta)t, \Omega) $\;
				}
			\end{algorithm}
		\end{minipage}
	}
...
\begin{frame}{Final result}
	Combining Luks's algorithm with the Babai-Luks reduction from $ \GI $ to $ \SI $ for $ \Gamma_d $ groups yields:

...
\begin{frame}{Proof of Luks's theorem (continued)}
	...
	\end{align*} is well-defined. By induction hypothesis $ \Aut(R_{i-1}, v) \in \Gamma_d $ and thus \[
		\Aut(R_i, v)/\KernelOf(\varphi_i) \cong \ImageOf(\varphi_i) = \Aut(R_{i-1}, v) \in \Gamma_d
	...
	\begin{columns}
		...
		\begin{column}{0.65\textwidth}  %%<--- here
			...
			\begin{itemize}
				...
				\item $ \rightsquigarrow $ If $ \sigma(w_1) = w_2 $ for some $ \sigma \in \KernelOf(\varphi_i) $ then $ w_1, w_2 \in W $ and $ N(w_1) = N(w_2) $
				\pause
				...
				\item Since $ \Gamma_d $ is closed under taking subgroups and $ S_d^m \in \Gamma_d $, the result follows.
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Solving $ \GI$ for bounded-degree graphs}
	By combining this result with the idea of the reduction $ \GI \le_m^p \SI $ that we proved earlier, alongside other insights, one can show: \begin{theorem}[Babai, Luks]
		There is a polynomial-time Turing reduction from $ \GI $ for graphs of maximum degree $ d $ to $ \SI $ for a group $ \Gamma \in \Gamma_d $.
	\end{theorem}

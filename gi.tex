...
\begin{frame}{Solutions to $ \GI $ have a very nice structure}
	\only<1-2|handout:1>{Consider the isomorphisms between the following graph and itself:
	\begin{center}
		\solSpaceExGraphZero
	\end{center}}
	\only<2|handout:1>{For example:\begin{center}
		\renewcommand{\arraystretch}{1.5} % row height
		\begin{tabular}{c c c}
		...
		\end{tabular}
	\end{center}}
	\only<3|handout:2>{But this means we can repeat the process:\begin{center}
			\renewcommand{\arraystretch}{1.5} % row height
			\begin{tabular}{c c c}
			...
			\end{tabular}
	\end{center}\begin{itemize}
	\item $ \rightsquigarrow $ The set of isomorphisms between a graph $ G $ and itself forms a group called \textit{automorphism group} and denoted $ \Aut(G) $
		...
		\begin{columns}
			...
			\begin{column}{0.4\textwidth}  %%<--- here
				\begin{tabular}{>{\centering\arraybackslash}m{2cm} >{\centering\arraybackslash}m{2.5cm}}
					Orbit&Stabilizer\\
					\hline\orbitSix&$ S_3 $\\
					\hline\orbitSeven&$ \gen{(1\,2\,3\,4),(1\,3)} $\\
					\hline\vspace{5pt}\orbitEight&$ \Z_2 \times \Z_2 $\\
					\hline\vspace{5pt}\orbitNine&$ \Z_2 $\\
					\hline\orbitTen&$ \gen{(1\,2\,3\,4),(1\,3)} $\\
					\hline\orbitEleven&$ S_3 $
				\end{tabular}
			\end{column}
		\end{columns}
	\end{center}
...
\begin{frame}{Graphs of bounded degree -- example}
	Consider connected graphs with maximum vertex degree $ \le 2 $:
	\begin{center}
		\setlength{\extrarowheight}{3pt}
		\vspace{-2pt}\begin{tabular}{>{\centering\arraybackslash}m{2cm} >{\centering\arraybackslash}m{1.3cm}|>{\centering\arraybackslash}m{5cm} >{\centering\arraybackslash}m{2cm}}
			Orbit of $ G $&$ \Aut(G) $&Composition series of $ \Aut(G) $&Composition factors\\
			\hline\vspace{5pt}\orbitFour&$ \Z_2 $&$ \{\id\} \trianglelefteq \Z_2 $&$ \Z_2 $\\
			\hline\orbitSeven&$ \gen{r,(1\,3)} $&$ \{\id\} \trianglelefteq \gen{r^2} \trianglelefteq \gen{r} \trianglelefteq \gen{r,(1\,3)} $&$ \Z_2, \Z_2, \Z_2 $
		\end{tabular}
	\end{center}\begin{itemize}
		\item Here, $ r := (1\,2\,3\,4) $ is the rotation permutation
		\item \textbf{Note}: All composition factors are bounded by $ 2 $
	\end{itemize}\pause
	\textcolor{MagentaZB}{\textbf{Key fact}}: This is not a coincidence: \begin{theorem}[Luks]
		Let $ d \ge 2 $. For any finite graph $ G $ of maximum degree at most $ d $ and $ v \in V(G) $, all composition factors of $ \Aut(G, v) $ are subgroups of $ S_d $.
	\end{theorem}
\end{frame}
...
\begin{frame}{Luks's algorithm -- high level overview}
	\textbf{General idea}: Recursively split the strings $ u $, $ v $ of the $ \SI $ instance according to the structure of the group action.
...
\begin{frame}
	\begin{center}
		\begin{tikzpicture}
			...
			\draw [decorate, decoration = {brace, raise=5pt, amplitude=5pt}] (utl) -- (uomega1sept) node[pos=0.5,above=10pt,black]{$ \Omega_1 $};
			\draw [decorate, decoration = {brace, raise=5pt, amplitude=5pt}] (uomega1sept) -- (uomega2sept) node[pos=0.5,above=10pt,black]{$ \Omega_2 $};
			\draw [decorate, decoration = {brace, raise=5pt, amplitude=5pt}] (uomegam1sept) -- (utr) node[pos=0.5,above=10pt,black]{$ \Omega_s $};

...
\begin{frame}{Graphs of bounded degree -- example}
	...
	\textcolor{MagentaZB}{\textbf{Key fact}}: This is not a coincidence: \begin{theorem}[Luks]
		Let $ d \ge 2 $. For any finite graph $ G $ of maximum degree at most $ d $ and $ v \in V(G) $, the composition factors of $ \Aut(G, v) $ are subgroups of $ S_d $.
	\end{theorem}
\end{frame}
...
\begin{frame}{$ \widehat{\Gamma}_d $-groups}
	...
	\begin{definition}[Luks]
		For $ d \ge 2 $ define  to be the class of all groups $ \Gamma $ such that every composition factor of $ \Gamma $ is isomorphic to a subgroup of $ S_d $.
	\end{definition}

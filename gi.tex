...
\begin{frame}{Solutions to $ \GI $ are cosets}
	\begin{itemize}
		\item Let $ \cdot $ denote the the vertex renaming operation of $ \Sym(V(G)) $ on the set of all graphs with vertices $ V(G) $
		\item TODO
	\end{itemize}
	\begin{align*}
		\{G_1 \mid G_1 \cong G_0\} &= S_n \cdot G_0 \\
		&= S_n \cdot (\Stab(G_0) \cdot G_0) \\
		&= S_n \cdot (\Aut(G_0) \cdot G_0) \\
		&= (S_n \circ \Aut(G_0)) \cdot G_0
	\end{align*}
\end{frame}
...
\begin{frame}{Blocks of group actions}
	\begin{columns}
		\begin{column}{0.4\textwidth}
...
\begin{frame}{Blocks of $ \Gamma = D_6 $ acting on $ \Omega := \{1,2,3,4,5,6\} $}
	A \textit{block} is a set $ \varnothing \neq \Delta \subseteq \Omega $ such that for every $ \gamma \in \Gamma $\begin{itemize}
		\item $ \gamma \cdot \Delta = \Delta $ \textbf{or}
		\item $ \gamma \cdot \Delta \cap \Delta = \varnothing $
	\end{itemize}
	In the present example $ \{2, 4, 6\} $ and $ \{1, 4\} $ are blocks:
	\begin{center}
		\begin{tikzpicture}[scale=1, thick]
...
\begin{frame}{TODO}
	\begin{theorem}[Babai, Cameron, P\'{a}lfy]
		For every $ d \in \Npos $ there exists $ c \in \R $ such that every primitive $ \Gamma_d $-group $ \Gamma \le S_n $ has order at most $ n^c $.
	\end{theorem}
\end{frame}
